...
static void cmdline_filter(struct tag *cmdline_tag, char *default_cmdline)
{
	const char *desired_cmds[] = {
                                     ...
                                     "lk_t=",
                                     "pl_t=",
                                     "androidboot.uboot_ver=",
                                     "androidboot.preloader="
			             };
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	else if (tags->hdr.tag == ATAG_BATT) {
		batt_vol = tags->u.batt.batt_vol;
	}
    }
    /*
...
__init int mt_board_init(void)
{
...

	{
		...
		if (key != 0)
			...
			memcpy(serial_number, "01234567891", 11);//randy modify for GOTA SNID
