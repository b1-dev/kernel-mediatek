...
static void mt6577_battery_update(struct mt6577_battery_data *bat_data)
{
	...
	int i;
    static bool battery_before_fail = false; 
...
#endif
    {
        if ( BMT_status.bat_exist )
        {
            ...
            else
            {
                ...
                if (battery_before_fail == true && BMT_status.SOC > bat_volt_check_point)
                {
                    xlog_printk(ANDROID_LOG_INFO, "Power/Battery_XXXX","battery_before_fail == true && BMT_status.SOC > bat_volt_check_point\r\n");
                    gSyncPercentage=0;
                    battery_before_fail = false;
                }
    ...
    else
    {
       ...
       if ((upmu_is_chr_det()==KAL_TRUE) && (g_Battery_Fail == true) && (g_Charging_Over_Time==0) && (usb_is_discharging_det() == 0))
       {
           xlog_printk(ANDROID_LOG_INFO, "Power/Battery_XXXX","g_Battery_Fail == true and set battery_before_fail = true\r\n");
           battery_before_fail = true;
       }else
       {
           xlog_printk(ANDROID_LOG_INFO, "Power/Battery_XXXX","g_Battery_fail == false and only battery\r\n");
           battery_before_fail = false;
       }
#ifdef MTK_NCP1851_SUPPORT
		...
		else if ( (gFG_voltage <= gFG_15_vlot)&&(gForceADCsolution==0)&&(bat_volt_check_point>=10) )
		{
			/*Use gas gauge*/
			gSyncPercentage=1;
...
//       ncp1851_set_int_mask(0x0); //disable all interrupt
       ...
       if((ncp1851_status == 0x8) || (ncp1851_status == 0x9) || (ncp1851_status == 0xA)) //WEAK WAIT, WEAK SAFE, WEAK CHARGE
           ...
           ncp1851_set_ctrl_vbat(0x24); //VCHG = 4.2V for 2nd battery (set 0x26 for 4.25V)
       else
...
#if 0	
       //Set SPM = 0, for acer platform, GPIO109
       mt_set_gpio_mode(GPIO109, GPIO_MODE_00);
       mt_set_gpio_dir(GPIO109, GPIO_DIR_OUT);
       mt_set_gpio_out(GPIO109, GPIO_OUT_ZERO);
#endif	   
}
#endif
		    ...
		    pchr_turn_off_charging_ncp1851();
              ...
              else
              {
                  if((gFG_booting_counter_I_FLAG == 2) || (g_ocv_lookup_done == 1))
                  {
                      ...
                      //Set SPM = 1, for acer platform, GPIO109
                      mt_set_gpio_mode(GPIO109, GPIO_MODE_00);
                      mt_set_gpio_dir(GPIO109, GPIO_DIR_OUT);
                      mt_set_gpio_out(GPIO109, GPIO_OUT_ONE);
                      if (Enable_BATDRV_LOG == 1) {
...
int BAT_CheckBatteryStatus_ncp1851(void)
{
    ...
    if((g_battery_tt_check_flag == 0) && (BMT_status.temperature < 75) && (BMT_status.temperature > (-20)))
    {
        g_battery_thermal_throttling_flag = 1;
...
void BAT_thread_ncp1851(void)
{
 ...
 #else
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery:ncp1851] Tbat(%d)>=75, system need power down.\n", BMT_status.temperature);
            mt6577_battery_update_power_down(&mt6577_battery_main);
            if( upmu_is_chr_det() == KAL_TRUE )
            {
                // can not power down due to charger exist, so need reset system
    ...
    while(1)
    {
        if( upmu_get_PCHR_CHRDET() == KAL_TRUE ) //do not check USB device check at precharge state
        {
            ...
            if((ncp1851_status == 0x8) || (ncp1851_status == 0x9) || (ncp1851_status == 0xA)) //WEAK WAIT, WEAK SAFE, WEAK CHARGE
                ...
                ncp1851_set_ctrl_vbat(0x24); //VCHG = 4.2V (set 0x26 for 4.25V)
            else
                ncp1851_set_ctrl_vbat(0x24); //VCHG = 4.2V
            ...
            if((g_battery_tt_check_flag == 0) && (bat_temp < 75) && (bat_temp > (-20)))
            {
                g_battery_thermal_throttling_flag = 1;
