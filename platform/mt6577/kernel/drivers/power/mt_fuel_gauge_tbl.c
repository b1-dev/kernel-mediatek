...
void fgauge_get_profile_id(void)
{
	...
	//Get battery profile id, for acer platform, GPIO99
	mt_set_gpio_mode(GPIO99, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO99, GPIO_DIR_IN);
	ret = (int)mt_get_gpio_in(GPIO99);

	printk("[fgauge_get_profile_id] battery id GPIO = %d\n", ret);
	...
	if((fg_battery_id != 1) && (fg_battery_id != 2)) {
...
#else
int r_fg_offset=25; //modify to 25 to fit PCB parasitic resistance
#endif
...
kal_int32 fgauge_read_capacity(kal_int32 type)
{
	...
	if (batt_vol >= 4100 ) {
		compal_solution = 1;
		dvalue = fgauge_get_dod0(batt_vol, temperature, KAL_FALSE);
	} else {
		compal_solution = 0;
	...
	printk ("ATAGS : lk_voltage = %d, Compal solution = %d\n", batt_vol, compal_solution);
    }
...
//4. update DOD0 after booting Xs
	if(gFG_booting_counter_I_FLAG == 1)
	{
		...
		gFG_15_vlot = fgauge_read_v_by_capacity(91); //9%
