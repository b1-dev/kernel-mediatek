...
#define STK8311_DRIVER_VERSION  "10"   //10 = v1.0
/*----------------------------------------------------------------------------*/
...
static int stk8311_local_init(void);
static int stk8311_remove(void);
static int stk8311_init_flag = 0; // 0<==>OK -1<==>fail
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info stk8311_init_info = {
	.name = "stk8311",
	.init = stk8311_local_init,
	.uninit = stk8311_remove,
};
/*----------------------------------------------------------------------------*/
...
{
        ssize_t res;
        res = snprintf(buf, PAGE_SIZE, "%s", STK8311_DRIVER_VERSION);
        return res;
}
/*----------------------------------------------------------------------------*/
...
static int stk8311_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = stk8311_get_cust_acc_hw();
	...
	if(err = stk8311_create_attr(&stk8311_init_info.platform_driver_addr->driver))
	{
...
static int stk8311_i2c_remove(struct i2c_client *client)
{
	...
	if(err = stk8311_delete_attr(&stk8311_init_info.platform_driver_addr->driver))
	{
...
{
	struct acc_hw *hw = stk8311_get_cust_acc_hw();
	GSE_FUN();
...
//	}
	...
	hwmsen_gsensor_add(&stk8311_init_info);
