...
#define KXTIK1004_DRIVER_VERSION  "10"     //10 = v1.0
/*----------------------------------------------------------------------------*/
...
static int kxtik1004_init_flag = 0; // 0<==>OK -1<==>fail
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
{
        ssize_t res;
        res = snprintf(buf, PAGE_SIZE, "%s", KXTIK1004_DRIVER_VERSION);
        return res;
}
/*----------------------------------------------------------------------------*/
...
static int kxtik1004_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = kxtik1004_create_attr(&kxtik1004_init_info.platform_driver_addr->driver)))
	{
...
static int kxtik1004_i2c_remove(struct i2c_client *client)
{
	...
	if((err = kxtik1004_delete_attr(&kxtik1004_init_info.platform_driver_addr->driver)))
	{
