...
#define MC32X0_DRIVER_VERSION  "10"   //10 = v1.0
/*----------------------------------------------------------------------------*/
#define MC32X0_8G_14BIT
...
static int mc32x0_local_init(void);
static int mc32x0_remove(void);
static int mc32x0_init_flag =0; // 0<==> OK -1<==>fail
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info mc32x0_init_info = {
    .name = "mc32x0",
    .init = mc32x0_local_init,
    .uninit = mc32x0_remove,
};
/*----------------------------------------------------------------------------*/
...
static int MC32X0_Init(struct i2c_client *client, int reset_cali)
{
	...
	if (fd_file == NULL) 
	{
		GSE_LOG("%s: fail to open %s\n", __FUNCTION__, CALIB_PATH);
		cali_data[0] = 0;
		cali_data[1] = 0;
		cali_data[2] = 0;	
	}
	else
	{
		...
		closeFile(fd_file); 
		sscanf(buf, "%x,%x,%x\n",&cali_data[MC32X0_AXIS_X], &cali_data[MC32X0_AXIS_Y], &cali_data[MC32X0_AXIS_Z]);
		cali_data[MC32X0_AXIS_X] = cali_data[MC32X0_AXIS_X] * gsensor_gain.x / GRAVITY_EARTH_1000;
		cali_data[MC32X0_AXIS_Y] = cali_data[MC32X0_AXIS_X] * gsensor_gain.y / GRAVITY_EARTH_1000;
		cali_data[MC32X0_AXIS_Z] = cali_data[MC32X0_AXIS_X] * gsensor_gain.z / GRAVITY_EARTH_1000;	
...
/*
 * This function is to read the product id info
 */
static int MC32X0_Read_Product_ID(struct i2c_client *client , char *buf)
{
	struct mc32x0_i2c_data *obj = (struct mc32x0_i2c_data*)i2c_get_clientdata(client);
	int res = 0;
	int retVal = 0;
	u8 databuf[2];
	databuf[1] = MC32X0_MODE_DEF;
	databuf[0] = MC32X0_Mode_Feature_REG;
	res = i2c_master_send(client, databuf, 0x2);
	if (!buf || !client)
	{
	...
	{
		GSE_ERR("I2C Error!\n");
		retVal = MC32X0_ERR_I2C;
	}
	/* get the product code */
	if(hwmsen_read_block(client, MC32X0_PRODUCT_CODE_REG, databuf, 1) <= 0)
	{
		GSE_ERR("I2C read product code register error!\n");
		retVal = MC32X0_ERR_I2C;
	}
	switch (databuf[0])
	{
		case MCUBE_3210:
			GSE_LOG("MCube Gsensor Prodcut MCUBE_3210!\n");
			retVal = MCUBE_3210;
		case MCUBE_3230:                   
			GSE_LOG("MCube Gsensor Prodcut MCUBE_3230!\n");
			retVal = MCUBE_3230;           
		case MCUBE_3250:                   
			GSE_LOG("MCube Gsensor Prodcut MCUBE_3250!\n");
			retVal = MCUBE_3250;           
		case MCUBE_3410:                   
		    GSE_LOG("MCube Gsensor Prodcut MCUBE_3410!\n");
			retVal = MCUBE_3410;           
		case MCUBE_3430:                   
			GSE_LOG("MCube Gsensor Prodcut MCUBE_3430!\n");
			retVal = MCUBE_3430;
		default:
			GSE_ERR("MCube Gsensor Prodcut Check Failed!\n");
			retVal = MC32X0_ERR_IDENTIFICATION;
	}
	sprintf(buf, "%04x", retVal);
	return retVal;
}
/*----------------------------------------------------------------------------*/
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
    GSE_LOG("fwq show_cali_value \n");
        ...
        obj = (struct mc32x0_i2c_data*) i2c_get_clientdata(client);
        if((err = MC32X0_ReadOffset(client, obj->offset)))
        {
                return -EINVAL;
        }
        ...
        {   
                ...
                mul = gsensor_gain.x /mc32x0_offset_resolution.sensitivity;
...
/*----------------------------------------------------------------------------*/
static ssize_t show_prodinfo_value(struct device_driver *ddri, char *buf)
{
	GSE_LOG("fwq show_prodinfo_value \n");
	struct i2c_client *client = mc32x0_i2c_client;
	char strbuf[MC32X0_BUFSIZE];
	if(NULL == client)
	{
	...
	MC32X0_Read_Product_ID(client, strbuf);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);        
}
/*----------------------------------------------------------------------------*/
...
{
        ssize_t res;
        res = snprintf(buf, PAGE_SIZE, "%s", MC32X0_DRIVER_VERSION);
        return res;
}
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(prodinfo,             S_IRUGO, show_prodinfo_value,      NULL);
...
static struct driver_attribute *mc32x0_attr_list[] = {
	...
	&driver_attr_status,
	&driver_attr_power, 
	&driver_attr_prodinfo,	   /*product information*/
        &driver_attr_version,
};
/*----------------------------------------------------------------------------*/
...
static int mc32x0_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = mc32x0_create_attr(&(mc32x0_init_info.platform_driver_addr->driver)))
	{
...
{
	int err = 0;	
	if(err = mc32x0_delete_attr(&mc32x0_init_info.platform_driver_addr->driver))
	{
...
{
	struct acc_hw *hw = mc32x0_get_cust_acc_hw();
	GSE_FUN();
	MC32X0_power(hw, 1);
...
static int __init mc32x0_init(void)
{
	...
	struct acc_hw *hw = mc32x0_get_cust_acc_hw();
	i2c_register_board_info(hw->i2c_num, &i2c_mc32x0, 1);
	hwmsen_gsensor_add(&mc32x0_init_info);
	return 0;
}
